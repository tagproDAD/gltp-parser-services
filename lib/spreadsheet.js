...
async function fetchSpreadsheet() {
  ...
  console.log("SS", SHEET_URL); // ✅ log parsed object
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Use the exact header name from your sheet
    const mapId = row["Map Id"];  

    configs[mapId] = {
      caps_to_win: Number(row["caps_to_win"] || 1),
      allow_blue_caps: row["allow_blue_caps"]?.toUpperCase() === "TRUE",
    };
  }

  console.log("Parsed configs:", configs); // ✅ log parsed object
  cache = configs;
  lastFetch = now;
  return configs;
}
